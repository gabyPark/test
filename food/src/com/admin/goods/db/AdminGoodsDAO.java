...
//------------------------------------------------------------------------------------- void insertGoods(GoodsBean gb)
	...
	public void insertGoods(GoodsBean gb){ /* 작업 내용 : 판매할 상품 상세 정보를 디비 테이블에 넣는다 */
		...
		try {
			...
			if(rs.next()){
				num = rs.getInt(1)+1; // rs.getInt(1)의 값이 null이라면(=게시글 없음) 0을 반환한다.
			}
		
			// 디비에 상품 정보 저장하기(insert into)
			SQL = "insert into f_goods"
					+ " values(?,?,?,?,?,now(),?,?,?)";
			pstmt = con.prepareStatement(SQL);
			pstmt.setInt(1, num);
...
//------------------------------------------------------------------------------------- void insertGoods(GoodsBean gb)
	...
	public List<GoodsBean> getAdminGoodsList(String item){  /* 작업 내용 : 디비에 저장되어 있는 모든 상품 데이터를 꺼내서 goodsBean -> List에 담는다*/
		...
		try {
			...
			if(item.equals("all")){
				...
				pstmt.setInt(1, 1); // 인기 상품의 경우는 1의 값을 가질 것이고, 일반 상품은 0의 값을 가질 것이라고 지정했기 때문에 1을 임의로 줄 수 있음
			}else{
