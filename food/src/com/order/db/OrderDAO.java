...
public class OrderDAO {
	...
	public void addOrder(OrderBean ob,ArrayList basketList,ArrayList goodsList){
		...
		try {
			...
			//3. sql( o_num 일련번호 계산 )
			sql = "select max(o_num) from f_order";
			...
			for(int i=0;i<basketList.size();i++){
				...
				pstmt.setString(6, ob.getF_or_mem_id());
				
				pstmt.setString(7, ob.getF_or_rec_name());
				pstmt.setString(8, ob.getF_or_rec_addr1());
				pstmt.setString(9, ob.getF_or_rec_addr2());
				pstmt.setString(10, ob.getF_or_rec_phone());
				pstmt.setString(11, ob.getF_or_memo());
				
				pstmt.setInt(12, bk.getF_bas_amount() * gb.getF_gs_price());
				pstmt.setString(13, ob.getF_or_trade_type());
				pstmt.setString(14, ob.getF_or_trade_payer());
				
				pstmt.setString(15, ""); // 운송장번호 - 주문시 공백문자
				pstmt.setInt(17, 0); // 주문상태
	...
	public List getOrderList(String id){
		...
		try {
			...
			con = getCon();
			//3. sql
			sql = "select f_or_trade_num,f_or_gs_name,f_or_gs_amount,"
					+ "sum(f_or_sum) as f_or_sum,"
					+ "f_or_trans_num,f_or_date,f_or_status,f_or_trade_type from f_order "
					+ "where f_or_mem_id=? group by f_or_trade_num order by f_or_trade_num desc";
			...
			while(rs.next()){
				...
				ob.setF_or_bas_amount(rs.getInt("f_or_gs_amount"));
